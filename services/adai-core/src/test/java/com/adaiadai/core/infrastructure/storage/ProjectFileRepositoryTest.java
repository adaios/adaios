...
class ProjectFileRepositoryTest {
    ...
    void saveAndFindAll_parsesAllEntries() {
        ...
        repository.save("default",task("task_1", "任务一", "描述一", TaskStatus.TODO));
        repository.save("default",task("task_2", "任务二", "描述二", TaskStatus.DONE));
        repository.save("default",task("task_3", "任务三", "描述三", TaskStatus.TODO));

        List<Task> all = repository.findAll("default");
    ...
    void saveAndFindAll_preservesFields() {
        ...
        Task loaded = repository.findById("default","task_f1").orElseThrow();
    ...
    void save_multilineTitle_isFlattenedToSingleLine() {
        ...
        repository.save("default",task("task_ml", "标题第一行\n标题第二行\nstatus: 污染", "正常描述", TaskStatus.TODO));

        Optional<Task> found = repository.findById("default","task_ml");
    ...
    void save_update_replacesEntryNotDuplicates() {
        repository.save("default",task("task_u", "原始", "原始描述", TaskStatus.TODO));
        repository.save("default",task("task_u", "更新后", "更新描述", TaskStatus.DOING));

        List<Task> all = repository.findAll("default");
        assertEquals(1, all.size(), "更新不应产生重复条目");
        Task loaded = repository.findById("default","task_u").orElseThrow();
    ...
    void findById_notFound_returnsEmpty() {
        assertFalse(repository.findById("default","task_none").isPresent());
    }

    @Test
    void delete_removesEntry() {
        repository.save("default",task("task_d", "待删", "描述", TaskStatus.TODO));
        repository.delete("default","task_d");
        assertTrue(repository.findAll("default").isEmpty());
    }

    @Test
    void delete_nonexistent_doesNothing() {
        repository.save("default",task("task_k", "保留", "描述", TaskStatus.TODO));
        repository.delete("default","task_none");
    ...
    void saveAndDelete_preservesManualComments() {
        ...
        repository.save("default",task("task_c1", "任务一", "描述一", TaskStatus.TODO));
        ...
        repository.save("default",task("task_c1", "任务一更新", "描述一", TaskStatus.DOING));
        String afterSave = fileStorage.read("default",path);
        assertTrue(afterSave.contains("手动说明：本周重点做 A 方向。"), "save 后手写注释应保留");
        assertEquals(1, repository.findAll("default").size());
        assertEquals("任务一更新", repository.findById("default","task_c1").orElseThrow().title());

        // delete 另一任务 → 重建文件也应保留注释
        repository.save("default",task("task_c2", "任务二", "描述二", TaskStatus.TODO));
        repository.delete("default","task_c2");
    ...
    void stats_countsByStatus() {
        repository.save("default",task("task_s1", "a", "1", TaskStatus.TODO));
        repository.save("default",task("task_s2", "b", "2", TaskStatus.DOING));
        repository.save("default",task("task_s3", "c", "3", TaskStatus.DONE));
        repository.save("default",task("task_s4", "d", "4", TaskStatus.CANCELLED));

        TaskRepository.TaskStats stats = repository.stats("default");
    ...
    void findAll_filterByStatusAndTag() {
        repository.save("default",task("task_f1", "后端任务", "1", TaskStatus.TODO));
        repository.save("default",new Task("task_f2", "前端任务", "2", TaskStatus.DONE,
                "P1", List.of("前端"), null, LocalDate.now(), LocalDate.now()));

        List<Task> byStatus = repository.findAll(TaskStatus.DONE, null, "default");
        assertEquals(1, byStatus.size());
        assertEquals("task_f2", byStatus.get(0).id());

        List<Task> byTag = repository.findAll(null, "前端", "default");
