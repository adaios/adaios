 ...
 * 覆盖触发规则（方案 B）：domain=project + intent=log + actionable + 无排除标签 + 幂等。
 */
class RecordToTaskLinkerTest {
    ...
    void setUp() {
        ...
        ProjectTaskAppService taskService = new ProjectTaskAppService(taskRepo);
        linker = new RecordToTaskLinker(taskRepo, taskService);
    }
