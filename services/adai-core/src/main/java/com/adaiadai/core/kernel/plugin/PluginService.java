 ...
 * seed adai = [trading]（owner 受控插件）。未知插件名过滤，防脏数据
 * （RFC 20260917 撤 project 后，存量账号里的 "project" 由 {@code isValid} 自动过滤）。
 * <p>
...
public class PluginService {
     ...
     * （无插件用户不应出现 trading 标注；知识/上下文注入已按插件门控。
     *   RFC 20260917：project 插件已撤，project 不再是合法 domain）
     */
    public String gateDomain(String userId, String domain) {
        ...
        // RFC 20260917：project 插件已撤，domain=project 不再是合法值，一律收敛 life
        if (!PluginRegistry.PLUGIN_TRADING.equals(domain) && !"life".equals(domain)) {
            return "life";
        }
        return domain;
    }
