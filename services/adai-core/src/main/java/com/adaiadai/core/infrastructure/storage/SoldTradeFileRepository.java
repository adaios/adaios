...
public class SoldTradeFileRepository implements SoldTradeRepository {
    ...
    public List<SoldTrade> findAll(String userId) {
        ...
        try {
            ...
            // RFC 20260909：provenance 读写（老文件缺字段 → asText 空串 → record canonical 默认 "import"）
    ...
    public void saveAll(String userId, List<SoldTrade> trades) {
        ...
        synchronized (lockFor(userId)) {
            writeAll(userId, trades);
        }
    }

    @Override
    public void upsertFromFlow(String userId, SoldTrade trade) {
        // ClearanceDetector 专属写入口：锁内读-改-写，只填空白 symbol（P1，防并发与导入互相丢更新）
        synchronized (lockFor(userId)) {
            List<SoldTrade> all = new ArrayList<>(findAll(userId));
            boolean exists = all.stream().anyMatch(t -> t.symbol().equals(trade.symbol()));
            if (exists) return; // 已存在（无论 provenance）→ 不写、不覆盖
            all.add(trade);
            writeAll(userId, all);
        }
    }

    /** 锁内全量写盘（调用方需已持有 lockFor(userId) monitor）。 */
    private void writeAll(String userId, List<SoldTrade> trades) {
        try {
