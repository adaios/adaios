 ...
 * <p>
 * 插件 = adai 拥有并受控开放的 Domain（trading / learn），启用载体 = Account.plugins
 * （adai-admin 后台控制）。Kernel 基础服务（记录/问答/记忆/档案/时间线/搜索/待办）不是插件，
 * 人人都有，不在此表（RFC 20260917 三层定位：core / builtin / optional）。
 * <p>
...
public class PluginRegistry {
    ...
    private static final Set<String> PLUGINS = Set.of(PLUGIN_TRADING, PLUGIN_LEARN);

    /** 该名称是否为已知插件（trading / learn）。life、待办等基础能力不是插件。 */
     ...
     * <p>
     * 按实现类简单名映射（trading 域 3 个）——Domain 贡献者不暴露插件元数据，
