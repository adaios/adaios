package com.adaiadai.core.application;

import com.adaiadai.core.domain.learn.LearnCard;
import com.adaiadai.core.domain.learn.LearnCardRepository;
import com.adaiadai.core.domain.trading.PushSettings;
import com.adaiadai.core.infrastructure.storage.PushSettingsRepository;
import com.adaiadai.core.kernel.account.Account;
import com.adaiadai.core.kernel.account.AccountRepository;
import com.adaiadai.core.kernel.plugin.PluginRegistry;
import com.adaiadai.core.kernel.plugin.PluginService;
import com.adaiadai.core.kernel.push.PushChannel;
import org.slf4j.Logger;
import org.slf4j.LoggerFactory;
import org.springframework.scheduling.annotation.Scheduled;
import org.springframework.stereotype.Service;

import java.time.LocalDate;
import java.util.ArrayList;
import java.util.List;

/**
 * LearnReviewPushService — 学习复习提醒推送（RFC 20260829 learn V2 消化闭环批 4）。
 * <p>
 * 每晚 20:00（用户拍板 2026-09-07）：遍历启用 learn 插件的用户，聚合「进入复习队列（review）
 * 已满 7 天仍未完成（done）」的卡片，推一条汇总到 Feed（type=learn-review，PushChannel 渠道化；
 * 开关走 PushSettings——缺失默认开，可关，learn 侧端点可独立关闭）。
 * <p>
 * S-learn1 口径修复（2026-09-07 learn V2 审查，用户拍板）：
 * <ul>
 *   <li><b>计时起点 = review_at</b>（进入 review 之日，由 updateStatus 写入），不再是消化日
 *       created——new 卡放一个月才点 review，不会次日就收到「满 7 天」假提醒；</li>
 *   <li><b>节流防 nag</b>：同一张卡 7 天内至多提醒一次（reminded_at 记上次提醒日；提醒后
 *       markReminded 写回）——搁置卡从「每晚都推 30 次」降为「每 7 天一次」。</li>
 * </ul>
 * <p>
 * 间隔复习语义（RFC 3.2「一周/一月回看卡片」的最简落地）：new → 用户复习 → review；review 满
 * 7 天未 done → 提醒。done = 你确认消化完毕。仅聚合提醒，不自动改状态。推送失败不阻断
 * （逐用户 catch，日志告警）。
 */
@Service
public class LearnReviewPushService {

    private static final Logger log = LoggerFactory.getLogger(LearnReviewPushService.class);

    /** 每晚 20:00（非交易日限定——学习不受行情日程约束）。 */
    static final String CRON_EVENING = "0 0 20 * * *";

    /** review 进入 7 天仍未 done 即提醒（用户拍板口径 2026-09-07）。 */
    static final int REVIEW_STALE_DAYS = 7;

    /** 提醒节流：同卡距上次提醒 ≥ 7 天才再次提醒（防每晚 nag，S-learn1 2026-09-07）。 */
    static final int REMIND_COOLDOWN_DAYS = 7;

    /** 推送标题（Feed 卡 + Bark 均透传；前端按标题分类/配色需同步）。 */
    static final String TITLE = "学习复习提醒";

    private final AccountRepository accountRepository;
    private final PluginService pluginService;
    private final LearnCardRepository learnCardRepository;
    private final PushSettingsRepository pushSettingsRepository;
    private final List<PushChannel> pushChannels;

    public LearnReviewPushService(AccountRepository accountRepository,
                                  PluginService pluginService,
                                  LearnCardRepository learnCardRepository,
                                  PushSettingsRepository pushSettingsRepository,
                                  List<PushChannel> pushChannels) {
        this.accountRepository = accountRepository;
        this.pluginService = pluginService;
        this.learnCardRepository = learnCardRepository;
        this.pushSettingsRepository = pushSettingsRepository;
        this.pushChannels = pushChannels;
    }

    /** 读取复习提醒开关（learn-review；缺失默认开）。 */
    public boolean reviewEnabled(String userId) {
        return pushSettingsRepository.findByUser(userId).isEnabled("learn-review");
    }

    /** 设置复习提醒开关（S-learn2 2026-09-07：learn 插件用户可自关，不经 trading 门控）。 */
    public void setReviewEnabled(String userId, boolean enabled) {
        PushSettings settings = pushSettingsRepository.findByUser(userId).with("learn-review", enabled);
        pushSettingsRepository.save(userId, settings);
        log.info("学习复习提醒开关已更新 | userId={} | enabled={}", userId, enabled);
    }

    @Scheduled(cron = "${adai.learn.review-cron:" + CRON_EVENING + "}")
    public void reviewReminder() {
        LocalDate today = LocalDate.now();
        for (Account account : accountRepository.findAll()) {
            String userId = account.userId();
            if (userId == null || !account.enabled()) continue;
            if (!pluginService.hasPlugin(userId, PluginRegistry.PLUGIN_LEARN)) continue;
            try {
                remindIfStale(userId, today);
            } catch (Exception e) {
                log.warn("学习复习提醒失败 | userId={} | {}", userId, e.getMessage());
            }
        }
    }

    /** 单用户：聚合 review 超 7 天未 done 且冷却期已过的卡片，有则推送并标记提醒日。 */
    private void remindIfStale(String userId, LocalDate today) {
        List<LearnCard> stale = staleReviewCards(userId, today);
        if (stale.isEmpty()) {
            return;
        }
        // 推送开关——learn-review 关闭则不推（learn 侧端点可关，纯 learn 用户可达）
        if (!pushSettingsRepository.findByUser(userId).isEnabled("learn-review")) {
            log.info("学习复习提醒跳过（用户关闭 learn-review）| userId={}", userId);
            return;
        }
        String content = buildContent(stale);
        PushChannel.PushMessage message = new PushChannel.PushMessage(
                TITLE, content, "learn-review", null, null, java.time.LocalTime.now());
        for (PushChannel channel : pushChannels) {
            if (channel.enabled()) {
                try {
                    channel.push(userId, message);
                } catch (Exception e) {
                    log.warn("学习复习提醒推送渠道失败 | userId={} | channel={} | {}",
                            userId, channel.name(), e.getMessage());
                }
            }
        }
        // 推送成功（至少尝试过）→ 逐卡写 reminded_at（节流：7 天内不再提醒）
        for (LearnCard card : stale) {
            try {
                learnCardRepository.markReminded(userId, card.type(), card.title(), today);
            } catch (Exception e) {
                log.warn("学习复习提醒标记失败 | userId={} | {} | {}", userId, card.title(), e.getMessage());
            }
        }
        log.info("学习复习提醒已推送 | userId={} | 待复习 {} 张", userId, stale.size());
    }

    /**
     * 跨 ai/trading/other 全扫，筛「该提醒」的卡：status=review 且 review_at ≤ today−7
     * （进入队列满 7 天，S-learn1：不再按消化日 created）且距上次提醒 ≥ 7 天（节流防 nag）。
     */
    List<LearnCard> staleReviewCards(String userId, LocalDate today) {
        List<LearnCard> stale = new ArrayList<>();
        LocalDate staleBefore = today.minusDays(REVIEW_STALE_DAYS);
        LocalDate cooldownBefore = today.minusDays(REMIND_COOLDOWN_DAYS);
        for (String type : List.of(LearnCard.TYPE_AI, LearnCard.TYPE_TRADING, LearnCard.TYPE_OTHER)) {
            for (LearnCard card : learnCardRepository.list(userId, type)) {
                if (!LearnCard.STATUS_REVIEW.equals(card.status())) continue;
                if (card.reviewAt() == null) continue; // 老数据缺 review_at → 不误提醒（宁可漏不烦）
                if (card.reviewAt().isAfter(staleBefore)) continue; // 进入队列未满 7 天
                if (card.remindedAt() != null && card.remindedAt().isAfter(cooldownBefore)) continue; // 冷却中
                stale.add(card);
            }
        }
        stale.sort(java.util.Comparator.comparing(LearnCard::reviewAt,
                java.util.Comparator.nullsLast(java.util.Comparator.naturalOrder())));
        return stale;
    }

    private String buildContent(List<LearnCard> stale) {
        StringBuilder sb = new StringBuilder();
        sb.append("有 ").append(stale.size()).append(" 张卡片进入复习队列已满 7 天还没看完，该回看了：\n");
        for (LearnCard card : stale) {
            sb.append("· ").append(card.title()).append("（进入复习 ").append(card.reviewAt()).append("）\n");
        }
        sb.append("看完记得把状态改成 done 哦。");
        return sb.toString();
    }
}
