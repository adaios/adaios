...
public class RecordController {
    ...
    public ResponseEntity<?> createRecord(
        ...
        if (request.cardId() != null && cardRepository.findById(userId, request.cardId()).isPresent()) {
            ...
            var duplicate = questionAppService.findDuplicateResend(userId, request.cardId(), request.content());
            if (duplicate.isPresent()) {
                ...
                return ResponseEntity.ok(new QuestionResponse(
                        "question", dup.recordId(), dup.summary(), dup.tags(), dup.rawResponse(), dup.domain()));
            }
     ...
     * （建卡/append 与流式端点共用 {@link QuestionAppService#ensureCardWithUserTurn}——双路径同口径）
     */
    private ResponseEntity<QuestionResponse> handleQuestion(String userId, ContentRecord record, String cardId) {
        if (cardId != null) {
            questionAppService.ensureCardWithUserTurn(userId, cardId, record.content(), record.createdAt());
        }
    ...
    // ── 2026-08-26 重发去重（REVIEW S-9）──
    // 判定/命中逻辑已迁移 QuestionAppService.findDuplicateResend（2026-08-30 流式批：
    // 与 AskStreamController 流式路径共用，防双路径实现漂移）。

    /** 包内共享：流式端点（AskStreamController）用同一请求体构造记录。 */
    static ContentRecord buildRecord(CreateRecordRequest request) {
