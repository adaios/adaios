...
public final class TradingImportParser {
    ...
    public static CashQuery parseCash(String content) {
        ...
        if (m.find()) {
            ...
            assets.value = parseNum(m.group(2));
        }
        int[] col = null;
        for (String line : lines) {
            ...
            if (col == null) {
                int[] idx = locate(cells, "证券代码", "证券名称", "证券数量", "成本价", "当前价", "浮动盈亏");
                if (idx[0] >= 0) col = idx;
                continue;
            }
            if (cells.length <= col[0] || !cells[col[0]].matches("\\d{6}")) continue;
        ...
        return new CashQuery(cash.value, assets.value, positions);
    }
    ...
    public record CashPosition(String symbol, String name, int quantity,
                               double costPrice, double currentPrice, double pnl) {}

    /** 资金查询结果：余额/资产 + 明细。 */
    public record CashQuery(java.math.BigDecimal cash, java.math.BigDecimal assets,
