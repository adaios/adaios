 ...
 * @param tradeTime 成交时间（2026-09-18，P0-交易59：截图表格「成交时间」列提取）——同代码/同方向/
 *                  同价/同量的**分单**只有它能区分（生产实据：000831 两笔各 200 股 @53.300，
 *                  10:03:44 与 10:04:09 被吞成一笔）；无 → null（退回原判定）
...
public record TradeLogCandidate(
    ...
    /** 兼容构造（2026-09-18 新增 tradeTime 之前的 11 参口径）：tradeTime 缺省 null。 */
    public TradeLogCandidate(String symbol, String name, String direction, BigDecimal price,
                             ...
                             String orderId, BigDecimal fee, String id) {
        this(symbol, name, direction, price, volume, tradeDate, null, source, complete, orderId, fee, id);
    }
    ...
    public TradeLogCandidate(String symbol, String name, String direction, BigDecimal price,
        ...
        this(symbol, name, direction, price, volume, tradeDate, null, source, complete, orderId, fee, null);
    }

    /** 换 id 的副本（落盘前发号 / 读取时给旧数据补号用；其余字段逐字保留）。 */
    public TradeLogCandidate withId(String newId) {
        return new TradeLogCandidate(symbol, name, direction, price, volume, tradeDate, tradeTime,
    ...
    public TradeLogCandidate(String symbol, String name, String direction, BigDecimal price,
                             Integer volume, LocalDate tradeDate, String source, boolean complete) {
        this(symbol, name, direction, price, volume, tradeDate, null, source, complete, null, null, null);
    }

    /** 9 参委派构造（2026-09-18，P0-交易59）：带 tradeTime、orderId/fee/id 缺省 null。 */
    public TradeLogCandidate(String symbol, String name, String direction, BigDecimal price,
                             Integer volume, LocalDate tradeDate, LocalTime tradeTime,
                             String source, boolean complete) {
        this(symbol, name, direction, price, volume, tradeDate, tradeTime, source, complete,
                null, null, null);
    }
     ...
     *
     * <p>2026-09-18（P0-交易59）**补成交时间维度**：价格维度救不了「同价同量」的分单——
     * 生产实测用户卖出 000831 两笔各 200 股、成交价同为 53.300（10:03:44 / 10:04:09），
     * 被吞成 1 笔（实际卖 400 股只记 200 股，少记 10660 元）。成交时间不同 → 判为不同笔；
     * 同一张图重复上传时时间一致，去重语义不变。
     */
    public boolean sameTrade(TradeLogCandidate other) {
        ...
        // 成交时间维度（2026-09-18，P0-交易59）：同价同量分单的唯一区分维度。
        // 任一方无时间 → 退回数量判定（不因缺时间而漏去重，重传同一张图仍能判重）。
        if (tradeTime != null && other.tradeTime != null && !tradeTime.equals(other.tradeTime)) {
            return false;
        }
