...
| 维护 | **记忆重建 / 记忆重补 / 卡片清理** | ⚠️ 这些端点不在管理鉴权下（P-be-01 安全，待修）|
...
| **admin 应移除（→用户端）** | 档案编辑（P-role-01）、记忆修正（P-role-02）、任务 CRUD（P-role-04）、记录删除（P-role-03）——个人数据写归 app/web |
...
- 🔴 **P-be-01（安全）**：记忆重建/重补/清理等维护端点不在管理鉴权下——伪造 X-User-Id 即可操作任意用户（需移 /admin/** 或加鉴权）
- ⚠️ **P-role-01/02/03/04**：个人内容编辑（档案/记忆/任务/记录删除）在 admin 重复——收敛到用户端
