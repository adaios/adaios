...
| B65 | 防重型机制（防重/防双计/配额/幂等）**必须有自检出口与失败可见**：机制状态读不到时不得降级继续跑（fail-closed + 显式逃生口），并提供可查状态端点/字段；修复此类问题必须同时给**存量回填**路径（本次锚定：`GET/PUT /trading/anchor` + `GET /trading/integrity`）| 锚定 fail-open 三次复发（账实一致性批 P0，2026-09-12）|
| B66 | 真实业务数据（成交/流水/事件）**永不因系统状态不准而丢弃**：记账与归属分离——归属不了就落原始记录 + 行级 rejected 明细 + 缺口上报，禁止 `catch → log.warn → skipped++` | 3 笔真实卖出静默消失（账实一致性批 P0，2026-09-12）|
| B67 | 幂等判定必须**对称且跨来源**：同一实体多写入来源（截图/手动/导出/批量）共用一套键（业务编号 + 指纹），命中即合并回填元信息；「有编号才查编号」这类分支不对称即为缺陷；同指纹不同时刻需可区分 | 同笔双落 + 600206 被卖成 −600 股（账实一致性批 P1，2026-09-12）|
...
| B74 | 外部模型（VLM/LLM）输出 → 结构化字段的链路必须有**显式格式契约**：要么让模型直接吐结构化数组，要么解析器显式列出它接受的每一种版式。**降级路径不得静默**——认不出来就要说出来（进 `dropped` / 显式失败），禁止「0 命中 = 当作本来就没有」 | GLM 把表格一行拆成多行 → 正则 0 命中 → 降级单笔 Schema → 一张 3 笔的截图只落 1 笔（P0-交易53，2026-09-17）|
| B75 | 去重/合并键的维度必须足以**唯一确定一条**（成交需 symbol + 方向 + **价格** + 数量）。键少一维就会**吞掉真实数据**，而且是静默的。测试必须双向：既测「重复能被去掉」，也测「不同的两条不能互吞」。取舍原则：**宁可多留一笔让人工删，也不静默丢掉真实数据** | `sameTrade` 少价格维度 → 同标的同向各 100 股的三笔真实成交被吞成 1 笔（P0-交易53 / P1-交易56，2026-09-17）|
