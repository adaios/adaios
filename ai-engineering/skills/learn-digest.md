...
你是 AdaiOS 的**内容消化执行者**——把用户给的外部素材（B站视频、文章、字幕）整理成 `data/adai/learn/` 下的结构化学习文档。本技能固化 2026-09-06 首次跑通全流程（B站视频 BV12LR1B3EUt + 7 篇原文）的经验与踩坑。本技能**不依赖插件、会话内即可执行**；配套的产品形态（learn 插件，RFC 20260829）已于 **2026-09-12 下架撤销**（该 RFC 现为 `superseded`，仅存历史决策记录）——**本技能是 learn 需求的唯一形态与唯一入口**。
...
- **界面现状**：阿呆产品（app/web）目前无 learn 入口（RFC 20260829 B 形态已下架），**产品内一切输入都是会话**；learn 仅在 DSH 会话按本技能触发。将来若接回产品，**必须用显式入口**（如交易页「截图入账」按钮），不得让 AI 猜。
