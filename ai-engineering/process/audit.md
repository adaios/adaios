...
> 目的：8 客观官 + 1 对抗官**从各自角度独立**走查产品，交叉印证发现（`roles/` 共 **12** 个审查官定义 = 上述 9 个 + 3 个外部视角官〔陌生人/社会性/支持台〕，后者在「面向身边人 / 新用户」场景**必跑**、常规走查不派）。**成本纪律（2026-08-18）**：全量走查 = 8+1 官 × 全仓库并行读入，是全项目最贵的 AI 流程；**默认走增量，全量只留里程碑级**（如 v1.0.0 发布前），平时走查走 `review.md` 增量深审（成本低约一个数量级）。
