...
### 4. 部署前门禁 —— 最硬的一道 ✅ adaios 已做（deploy-gate.sh）
...
### 5. 部署后 smoke —— 自动验证 ✅ adaios 已做（deploy-gate.sh GATE-AFTER）

```
部署完成 → curl 新端点 + 核心链路（feed/advice/memory PATCH…）→ PASS/FAIL 报告
```

### 6. 定时 audit —— 防休眠 ✅ adaios 已做（weekly-audit.sh + cron）
...
| 优先级 | 切入点 | 状态 | 价值 |
|:------:|:-------|:----:|:-----|
| P0 | 部署前门禁 | ✅ | 最硬闸门，不过关不部署 |
| P0 | 部署后 smoke | ✅ | 部署即验证 |
| P1 | 定时 audit | ✅ | 防审查休眠 |
| P1 | post-commit 自审 | ❌ | 提交即自审（低优先）|
| P3 | pre-push | ❌ | 协作才必需（单人可延后）|
