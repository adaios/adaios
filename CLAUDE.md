...
### 已完成
- **R2 记录↔任务关联 + A2 相机动作分析 Phase 1（2026-08-13，RFC `20260813-record-task-and-sports-analysis` implemented）** ✅：**R2** domain=project 记录自动转任务——方案 B（默认转 + AI `actionable` 挡 + `#备忘`/`#想法` 排除标签手动挡）：`Task` 加可空 `sourceRecordId`（`ProjectFileRepository` 正则 `\s*`→`[ \t]*` 修「空字段贪婪吞下一行」根因，旧任务文件向后兼容）+ `RecordToTaskLinker`（幂等查重 + best-effort 不阻塞记录）+ `RecordController.handleStatem` 挂点；**A2 Phase 1** 图片卡「分析动作」按钮（双端，复用 askMedia 预设问题，零后端改动），Phase 2 专用 type + 视频抽帧留 v2；api-spec v3.16 + freeze §2.11 + ideas 登记；后端 **370** · adai-app 81 · adai-web 42 全绿
