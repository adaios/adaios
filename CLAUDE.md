...
| **交易知识** | `os/trading-engine/` | `cd os/trading-engine && claude` | 课程整理、规则提炼、术语融合 |
...
docs/                  # 项目文档
  ...
  inbox/               #   待处理/待归档区（20260722-ai-context-design.md 为阿呆早期设计历史保留）
apps/
  adai-app/            #   Flutter 前端（Web / Android / iOS）
    scripts/             # 构建/部署脚本
...
- **完整包结构树见 `services/adai-core/CLAUDE.md`**（唯一维护点，本文件不复制——避免双源漂移，REVIEW P3 docs×7）
- 分层依赖规则（全局原则）：
...
| 方向 | Phase | 状态 |
|:-----|:------|:----:|
| B Project OS | Phase 2-4 (RFC跟踪+自举+前端任务面板) | ✅ 完成 |
| A 行情接入 | Phase 1 (上下文注入) + Phase 2 (主动推送) | ✅ 完成（2026-08-06）|
| C Life OS | Phase 0-3 | ⏸ 等数据积累 |
| 记忆系统进化 | Phase 1-5 (kind/主题合并/actionable/时效/降噪) | ✅ 完成 |
| v0.2.0 | 前端 actionable 闭环 + L5 行情嵌入 | ✅ 完成 |
| 多账号预留 | 全链路 userId 分层（v1.0.0 前置）| ✅ 完成（架构预留）|
| adai-admin 全栈 | MD11-16 账号/数据/系统/知识 + memory 修正 | ✅ 完成 |
| adai-app 即入口 | 砍掉 adai-entry，app 直接作为产品入口 | ✅ 已执行 |
| adai-web 桌面端 | 独立工程两套 UI：两栏壳 + 8 模块桌面形态 | ✅ 已完成 |
| 多模态图片记录 | L4 图片 → GLM-VLM 文本化 → 现有闭环 | ✅ 前后端完成 |
| 安全基线 | #127 最小封闭鉴权（v1.0.0 前置）| ✅ 完成 |
| 文档治理 | RFC 20260815：瘦身 + 单一事实源 | 🔄 执行中（本批）|
| v1.0.0 发布 | 发布顺延（先治理流程）| ⏸ 待用户决策 |

### 测试状态 / 运行环境
- 唯一事实源：`docs/reference/status.md`（/ship 时更新，本文件不复制数字）

### 关键文档
- `docs/architecture/product-roadmap.md` — 🚩 产品路线 v1.0.0（唯一蓝图，路线驱动开发）
- `docs/reference/status.md` — 测试数/端点数/运行环境唯一真相（RFC 20260815）
- `docs/reference/change-log.md` — 批次变更日志（历史归档）
- `docs/review/REVIEW.md` — 未修项滚动区（战略 + P0-P2）
