...
| 日期 | 模式 | 基线 | 派发角色 | 新增 | 修复 |
|:-----|:-----|:-----|:---------|:-----|:-----|
| 2026-08-29 | 收盘小结 + 流式基础设施批（用户拍板「先修复不需要拍板的」）| 工作树（B 类批后续）| 主会话 | 0 新问题（纯修复）| P2-用户3 ✅ 出表 + P2-用户2 🔄 部分落地（流式后端就绪）|
...
| P2-3 | ✅ 已修（2026-08-29：weekly-audit cron 已挂载——每周一 9:00 `--auto` 追加 /tmp/weekly-audit.log，脚本 dry-run 验证 W1-W5 跑通；与 guard-cost 日账同 crontab）。** **weekly-audit cron 未挂载（08-23 元审核归口，无法证实）**：`crontab -l` 被 macOS TCC 拦截；仓库内无任何挂载证据（无 plist/日志/setup）——「防审查休眠」依赖不可验证且可能从未生效的触发器 | `weekly-audit.sh:3` | 授权 TCC 后确认挂载或改 launchd | ⚠️ 保留（需用户授权 TCC）|
...
| P2-用户2 | 🔄 部分落地（2026-08-29：超时矩阵 S-9 + 模型分层 S-10 已修；本批流式后端基础设施就绪——StreamingAiClient 端口 + DeepSeekAiClient.streamGenerate（SSE delta 解析/逐块回调/失败抛错）+2 测试；**SSE 端点 + app/web 双端流式渲染为 ai-calling-governance 批 2 剩余，下一批**）。** **交互反馈慢（2026-08-26 用户反馈）**：AI 类请求（问答/建议）硬等后端调模型，2核4G 服务器结构性慢；前端 90s vs 后端最坏 120s 超时矩阵自相矛盾（S-9）治理方案未落地 | AI 调用链路 / `ai-calling-governance.md` | 治理方案修订落地：流式输出 + 超时矩阵修正（S-9）+ 慢接口降级路径 |
