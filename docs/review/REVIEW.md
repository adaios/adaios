...
> S-2（附图文本语义重复）已按产品决策「一次输入 = 一个事件」**展示层聚合出表**（时间线/Feed 图文一体 + 多轮 chat 单条，2026-08-15，`eb5e707`）；数据层整体化（一次输入落盘一条，动 freeze §2.1）**RFC `20260815-media-event-unification` approved，排入 v1.0.1**。S-3/S-4 已修复出表（批 Q）。
