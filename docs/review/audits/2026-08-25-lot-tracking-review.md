...
| # | 级 | 问题 |
|:-:|:--|:--|
| P2-批次1 | P2 | web `_showLots` 连点/双击无幂等守卫——慢响应逐个 showDialog 叠两层（F20/F22 类） |
| P2-批次2 | P2 | 行为标注只分析导入文件的最大日期——10 天窗口内多日成交只有最后一天被判亏损加仓/追高（对抗 P2-5） |
| P2-批次3 | P2 | app 批次行 9px/10px 超小字号——P2-UI5 红线复发（含底仓徽标 9px / 批次文本 10px，`trading_page.dart:1039,1052`），并入 P2-UI5（27→29 处） |
