...
1. **P1-2 空态、P1-3 / P1-4 术语与错误文案**——改动本身是纯文案、风险低，**卡在几个自造词的替换用词需用户拍板**（下表）。改文案时**不碰同名的代码标识符 / API**（那是另一件事）。

| 现有词 | 分布 | 建议替换 | 备注 |
|:--|:--|:--|:--|
| 「反哺」 | 主要 web（用户可见 11 处），如「反哺候选」「反哺入库失败」 | 提炼 / 沉淀 | app 端 0 处——learn 管理界面只在 web |
| 「候选」 | app 11 / web 15，如「今日截图候选 N 笔」 | 待确认的几笔 | 交易模块高频，改前先确认不改出歧义 |
| 「批次」 | app 6 / web 8，如「每一笔买入一个批次」 | 这笔 / 每笔 | 与「批次止损」等既有术语联动，需整体看 |
| 「快照」 | app 3 / web 11，如「去电脑端导一次持仓股快照」 | 券商导出的持仓表 | 用户实际动作 = 从券商 App 导出文件 |

另有无争议、可直接改的：`Kernel · Domain · 数据`（首页）、`插件加载失败，仅显示基础服务`、`学习功能还没开（learn 插件）`、`API 端点`、`API 请求失败`、`更新 OS 标记失败`、`内部标识在 plugin 字段`、`第 N 行：字段不足`（web）、`解析失败: …` / `反哺入库失败: …` 这类「内部动词 + 异常原文」错误提示。

2. **三处重复 UI 抽共享组件**：app `main_page.dart` 与 `pages/trading_page.dart` 各有一份同名 `_PushSettingsDialog`（本批已把开关集合同步为 10 项，但代码仍两份，未来仍会各自漂移）；web 第三份。结构问题，不影响使用。
3. ~~`status.md` 测试数未更新~~ → **已同步**（pre-commit 的 GUARD-ALIGN A2 强制：app 269→270，后端 1761 与另一会话同源）。本批增量：后端 **+4**、app **+1**。
4. `feature-reference.md` 可补一句推送设置的非 iOS 行为（pre-commit A3 只 WARN 未拦；本批未改端点，`api-spec.md` 无需动）。
5. **Android 装机未验证**（需真机）——新包已就绪，但「装得上、装完能打开」还没人试过。
6. 递手机测试（真人第一句话）、开号通路、三端空世界走一遍——见 §五。
