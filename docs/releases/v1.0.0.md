...
- **多账号（v1.0.0 提前）** ✅：全链路 userId 分层（`data/{userId}/`）+ adai-admin 账号管理 + **前端选号/切换**（`GET /api/v1/accounts/available` 无鉴权选号 + shared_preferences 记住上次账号 + adai-app World B「切换账号」/ adai-web 底部 `@userId` 点击切换）
