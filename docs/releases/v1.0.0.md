...
- **工程基建** ✅：版本机制 · 审核流程（/review 三档 + guard.sh）· deploy.sh · 测试全覆盖（后端 298 · 前端 118）
...
- **REVIEW.md P1 已清零**（批 J：#144 rebuild 幂等 / #147 SELL 报错+锁 / #106 portfolio 契约 / #112 CANCELLED / #150 动态计数）
- **偶发 CanvasKit 渲染崩溃（Web，非阻塞）**：AI 回复 markdown 渲染（`MarkdownBody selectable`）偶发触发 wasm `Picture._cullRect` 崩溃——非必现、不丢数据、**刷新即恢复**（卡片 active 状态落盘，刷新后续 [结束] 可补生成 summary/tags）；建议后续版本评估 AI 回复渲染降级（去 selectable / 换 SelectionArea）
- **战略缺口（未修，不阻塞）**：#101 Feed 无「加载更早」分页 · #103 Timeline/Memory 保活数据陈旧 · #129 知识反哺闭环零产物（依赖真实复盘数据积累）
