...
| 前端 adai-app | **120** | 0 | Feed 状态机 12 + 6 页面 + 选号/切换链路 + 插件门控 + 带图发图即对话 + 交易建议 UI + 交易止损/买点 P0 + 交易对齐 web（账户总览卡/止损显示/默认-7%止损/自选买点/清仓打分）+ 30分钟自动刷新 + 次级数据代际守卫 + 上传批次锁 + 第一原则徽章移除（无记录/提问/领域系统标签）+ 20260818 简化（纯买卖记录去止损/买点 + 价格小键盘小数点）+ **2026-08-23 app 体感批**（IndexedStack 保活/排序四修/Feed 错误态/任务编辑 PUT/launcher 伪0/三态/人话/代际令牌/方向色/时间戳）|
