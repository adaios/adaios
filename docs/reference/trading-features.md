...
| 时间 | 任务 | 内容 |
|:--|:--|:--|
| 09:15 | **早盘计划（含买点，RFC 20260922 B1）** | 持仓概览（昨收/数量/成本/止损/择时）+ **自选买点逐条四要素铁证**（① 你的历史 ② 数字证据链 ③ 规则逐字原文 ④ 位置）；正文**确定性渲染、不再走 LLM**（四要素必须逐条可指认，RFC §八 1）；涉及账的文案标**账日期**（「按你 09-19 的账」）；行情双源失败 → 整条显式降级（B4）；买点段受 `buy-point` 开关门控（关掉即不扫不渲染）；判定数据陈旧（早于上一交易日）不推 |
| 12:00 | **午间知会（B5 / 用户拍板 D1）** | **知会不是提醒**：只报事实与位置（现价 / 今日涨跌 / 是否已到你设的止损位）+ 一句「要不要动留到尾盘那条说」；**不出现「快卖/建议减仓」这类催促**；**无异常（没触止损、没持仓）不发**；行情取不到不发（不发即不误导） |
| 14:50 | **尾盘卖点（RFC 20260922 B2）** | **只列触发卖出条件的持仓**（破止损 R66 / 超仓位 R81），逐票四要素铁证 + **账日期标注**；没有触发的票不发（无信号不制造噪音）；行情双源失败 → 显式降级「这条我暂时给不了」；建议照旧落 `advice-history`（A3 依据快照） |
...
| ~~15:10~~ | ~~收盘买点扫描~~ **已于 2026-09-22（B1）取消独立推送** | 用户明确「买点提醒并入早盘」——他是早盘买的人，昨日收盘触发的信号该在他要动手的时刻出现。扫描能力保留（早盘买点段与 `GET /trading/buy-points`、`GET /trading/buy-points/scan` 共用同一份判定） |
...
| **四要素铁证（RFC 20260922 B 批，2026-09-22）** | 每条决策提醒必须掏得出四条：① **本人历史操作统计**（`sold.json` 按形态 / 盈亏区间 / 持仓时长分桶，**最小样本 N≥5**，不足则明说「样本还不够」）· ② **具体数字证据链**（现价 / 命中信号 / 用户自己的参数阈值）· ③ **规则依据原文**（`os/trading-engine` rules.md **逐字**引用，取不到就不给）· ④ **可回溯可追责**（正文给位置；实体是 `AdviceEntry.basis` 依据快照）。渲染归 `TradingDecisionNarrator`，**缺证据不发**（不编数字、不编规则） |
...
| 资金 Tab | 现金/总资产概览；转入/转出（金额+备注）；导入资金（现金+精确成本）；本金设置（累计净投入）；说明「现金是 R81 判定分母（总资产=持仓+现金）」；**资金曲线卡（2026-09-04）**——净值折线（总资产÷净投入；本金未设自动退画总资产）+ 峰值线 + 最大回撤（GET /trading/equity-curve，空态引导导入资金/记录交易） | 「转入」(绿)/「转出」(橙)/「导入资金」/「设置本金」弹窗 | POST `/trading/transfer`、POST `/trading/imports/cash`、PUT `/trading/principal`、GET `/trading/account`、GET `/trading/equity-curve` |
...
> **2026-08-22 精简**：移除「自选股·买点信号」「清仓复盘」两个只读区块（管理归 web，能力不删——买点提醒**自 2026-09-22（B1）起并入早盘推送**，完整清仓复盘在 web）。
> **2026-09-22（C 批 C3）推送卡形态**：早盘/尾盘的决策推送在 App 里**默认只露一句话结论**（标题 / 账日期 / 逐票结论行），四要素依据收在「看依据（N 只 · 四要素）」之后一点即开（`lib/utils/push_evidence.dart` + `feed_card.dart` 的 `_PushContent`）；没有四要素的推送（行情异动/操作确认/复习提醒）原样全文，行为不变。

| 区块 | 功能 | 操作 | 端点 |
|:--|:--|:--|:--|
| 账户总览卡 | 总资产/总盈亏（券商口径优先 `assets>0`，失败退回组合快照）；总盈亏=资产−本金（本金>0）；可用/可取/市值/当日盈亏/本金小字指标行；金额 ≥1 万显示「X.X万」；**日/周/月盈亏行（v3.68）**：三个区间各自的金额 + 比例（`pct` null → 只给金额，不写 0%），本月跨锚定日时附「本月自券商快照 YYYY-MM-DD 起可追溯」；**该行可点进「收益日历」（v3.69，P2-交易52）** | 进页自动加载；AppBar 刷新按钮；点日/周/月行 → 收益日历 | GET `/trading/account`、GET `/trading/portfolio`、GET `/trading/pnl-periods` |
...
| **资金（v3.69，2026-09-16，P2-交易52）** | 现金 / 总资产概览 + **转入 / 转出 / 设置本金**三个动作。**「导入资金」刻意不放**——用户拍板「导入不做，不方便」（通达信文件导入归电脑端，与上面「去 web 引导」同口径）。提交期间按钮禁用（连点守卫），金额预检与 web 同口径（NaN / Infinity / ≤0 一律拦下） | 「转入」/「转出」→ 金额 + 备注弹窗；「设置本金」→ 数值弹窗（只写本金，不动现金 / 持仓） | POST `/trading/transfer`、PUT `/trading/principal`、GET `/trading/account` |
| **收益日历（v3.69，2026-09-16，P2-交易52）** | 独立页（`pages/profit_calendar_page.dart`）：月历格子显示**每日盈亏**（红涨绿亏，格子内短金额）+ 本月合计 + 有数交易日数 + ◀▶ 切月；点某天 → 底部明细（当天盈亏 / 当天总资产）。数据只取 `dailyPnl` 一处口径（与日/周/月三档同源回放，**前端不做差分**）；缺收盘价的日子显示「—」，**不写 ¥0.00** | 交易页账户卡「今日 / 本周 / 本月」那一行点进去 | GET `/trading/equity-curve`（`dailyPnl` 字段） |
...
| 自选股 | **无**（2026-08-22 移除区块；买点提醒**自 2026-09-22（B1）起并入早盘推送**） | 可导入 + 删除单只（带确认）+ 买点信号列 |
...
- **账本「丢行 / 丢值不可见」族（P2-交易42/43/44/45/47/48，2026-09-14 账本可见性批闭合）**：①历史成交导入的 5 类解析丢弃（代码非 6 位 / 买卖标志非买卖 / 价格非数字 / 日期格式坏 / 有量无价）现在带**行号 + 原文 + 原因**回前端（`unparsed`），并顺手修掉「价格非数字 → NPE 炸整个导入」；②截图入账/一句话归集被没记的行（未成交状态 / 申购配号 / 占位代码 / 0 价）在 `dropped` 里逐行说明；③资金股份导入首行任一项读不成数字 → **400 拒绝导入并点名哪一项**（原来 `null` 会写进账户快照），明细丢行 → `unparsedRows`；④`PUT /trading/profile` 空串不再清空画像；⑤账户卡 `todayPnlSource` 标注当日盈亏来源与日期（券商口径 / 系统计算 / 未知不标）；⑥负成本口径核对——资金曲线「缺 K 线兜底」不再算出负市值、建议引擎 `pnlPercent` 负/零成本一律 null。**已按用户拍板（2026-09-14「券商负成本」）实施**：负成本持仓**卖出的旧仓成本用券商成本价（含负）**，当日有买入致盘前成本取不到时退回历史买入均价并如实附注。通用 `/records`、`/records/media` 的丢行明细尚未进 Feed 回执（服务端 WARN 已如实记录）。
...
环 2  ✅ 自动拉 前60+后30 交易日 K → 特征画像 + 后验窗口 → 落盘 JSON → web 画图（通达信风格：三副图 + 单日指标查看 + 窗口移动，已完成）
环 3  ✅ LLM 理解（aiInsight 生成 + 落盘，2026-08-30 完成；≥20 案例归纳后置）
环 4  ✅ 判定当下（特征归一化 + 相似度引擎 + match 端点，2026-08-30 完成；15:10 扫描接入二期）
环 1  ✅ 批量导入（2026-08-31：B1/B2 笔记批量标注——解析器 + 全 A 名称表 + 逐条幂等，用户 32 案例入库）
```
