...
- **端点**：`POST /learn/digest`（喂入链接或素材，**2026-09-12 抓取批推荐端点**；`POST /learn/cards` 保留为兼容别名）、`POST /learn/digest/confirm`（**转写费用确认**，`{confirm:true|false}`，取消不花钱）、`GET /learn/digest/quota`（本月转写用量/费用/剩余额度 + 单价 + ASR 可用性）、`GET /learn/digest/status`（消化任务状态 idle/pending/running/needs_confirmation/done/failed/cancelled + stage/source/cost）、`POST /learn/cards`（喂入消化，**2026-09-10 起提交式**：POST 受理 `{status}` → 后台 AI 消化 → 轮询 `GET /learn/digest/status` 至 done/failed，不再同步阻塞几十秒超时；inflight 去重）、`GET /learn/digest/status`（消化任务状态 idle/running/done{type,title}/failed{message}，v3.56）、`GET /learn/cards`（`?type=` 列表）、`GET /learn/card`（`?type=&title=` 单篇全文）、`GET /learn/tree`（资产树，ai/trading/other 分组）、`PATCH /learn/cards/status`（复习状态流转 new/review/done，2026-09-07 V2）、`PATCH /learn/cards`（正文编辑补丁，?type=&title= 定位，2026-09-07 V2）、`POST /learn/cards/candidate`（trading 反哺候选，V2 批 3）、`GET /learn/cards/candidates`（候选列表）、`DELETE /learn/cards/candidates`（?title= 删除候选）。全需 learn 插件（403）。
...
- **抓取与转写边界（2026-09-12 抓取批更新口径）**：**已做**服务端抓取（B站视频 + 文章正文，域名白名单内）与云端转写（fun-asr，费用闸 + 用户确认前置）；**不做**YouTube（服务器网络不可达）与社交平台（公众号/知乎/小红书/X/微博/抖音，反爬与登录墙）的自动抓取——一律人话告知并引导用户粘正文/截图，不绕付费墙与登录墙；不做批量/后台自动转写（只在你明确发话时花钱）；**图片源（复用上传通道 + VLM）** 与**对话流喂入**仍在待排（RFC 20260912 阶段 2/3）。原「素材由用户喂入」表述作废（那是 B 形态的做法，已证明不可用）。- **其余已知边界（L1）**：**喂入双端页面入口（2026-09-10 提交式：web 资产页「＋」弹窗 + app「最近学习」页头「＋」）**——app 可随手粘贴素材消化（移动端主场），卡片编辑/复习流转仍在 web（app 只读呈现，双端分工）；trading 候选联动 V2 已落地（promote 融合仍走 trading-engine 工作流人工闸）；对话流喂入（阿呆对话里复述卡片）为批 2 待排（RFC 3.7 ① 主通道，前端接线未排）；多用户只架构预留；A 技能产物与插件产物同目录并存（目录结构差异由 L2 资产页统一）。
