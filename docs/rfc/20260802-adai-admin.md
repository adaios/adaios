...
- **seed 账号 `adai`**：首个管理员，文件预置（2026-08-02 定）
- 账号与数据分层联动：账号驱动 `data/{userId}/` 数据层（多账号架构预留已就绪）
- 登录状态：前端登录 → 携带 `X-User-Id`（预留全链路已就绪，只需功能层）
- **鉴权 ⏸ 后补**：v1 先做纯 userId 隔离（不防越权），口令/token 鉴权 app + 后端统一后补（2026-08-02 定）
...
| 版本归属 | v1.0.0，与多账号一起做 ✅ |
| 前端形态 | **独立入口**（2026-08-02）：adai-admin 独立构建/路由，复用 adai-app 设计系统 ✅ |
| seed 账号 | `adai`（首个管理员，文件预置）✅ |
| 鉴权 | ⏸ 后补：v1 纯 userId 隔离，口令/token 鉴权 app+后端统一后补 |
