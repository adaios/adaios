...
> ⚠️ **2026-09-12 当日晚间撤回（用户拍板：「我要的是完整的升级，成熟的方案」）**：本节的下架结论**不再执行**。
> - 用户要的是 learn **完整可用**，不是形态互斥/减法；§9.4 的六项执行动作**一项都不做**（不关插件、不删代码、不删测试、不删文档、`data/` 不动）。
> - learn 从此是**两个入口、一个契约**：Mac 侧 DSH 技能（深度整理：多源归并/概念追踪）+ 产品 app/web「学习」页（手机随手喂入：链接/图片 → 服务端抓取与转写）。**产物目录结构已统一**（2026-09-12 完整升级批：产品也落 `{type}/{topic}/NN-{slug}.md` + 主题 README + 主题内 `_raw/`）。
> - 本节保留为历史决策记录——**诊断依然有效**（「该做抓取，却把最费力的一步留给用户」+ 范围膨胀），新方向由 [`20260912-learn-product-digest.md`](20260912-learn-product-digest.md) 承接并已落地。
> - **仍然有效的约束**：§9.3「不砍的资产」（`data/adai/learn/**` 一篇不删；技能文档是单一事实源）。
>
> 以下为原始下架决策记录（**勿据此执行**）。
