...
date: 2026-08-15
status: approved
decided-by: adai（2026-09-22：明确「我要彻底打通这个链路，提升整个体验，带图对话的流程」——按 **A 方案**实施：一次投递（N 图 + 可选一句话）= 一个回合 = 一张卡；先识别多图组成上下文；无提问给一段综合总结、有提问据图作答；图并列同卡可继续追问）
depends-on:
...
> 触发：用户「我要彻底打通这个链路，提升整个体验，带图对话的流程」（2026-09-22）。本 RFC 由 draft 转 **approved**；下面**如实记下与本文档原方案的差异**（原方案写于 2026-08-15，部分取舍已被现实推翻）。

**已实施（后端 + 双端前端）**

1. **数据层先落地**（RFC `20260815-media-event-unification` step-1/2/4）：ContentRecord 新增可选 `mediaIds`；一次投递 = **1 条主记录**（带 `mediaIds`）+ N 条**薄 image 附件记录**（仅原图索引，不做 VLM、不沉淀记忆、不单独进 Feed）；Feed 条目新增 `mediaPaths`（`mediaPath` 仍为首图兼容）。
2. **入口**：新增 `POST /api/v1/records/media/batch`（multipart `files` 1-3 + `text` 可空 + 可选 `Idempotency-Key`）——**一次投递一次请求**；旧单图端点保留。
3. **视觉层**：新增 `VisualAiClient.understandMulti`（一次多图识别、综合总结 + 按序合并 OCR）——入账路径首次复用多图能力（此前 `askMulti` 只服务「已入库图片的问答」）。
4. **交互**：无提问/陈述 → 后端给**一段综合总结**（`type=image`）；问句 → **据图作答**（`type=image_qa`，回答在 `answer`，Q/A 追加首图卡）；前端一卡多图并列 + 卡内追问带**全部**图。

**与本文档原方案的差异（原取舍已推翻，以本节为准）**

| 原方案 | 现口径 | 依据 |
|:---|:---|:---|
| §四场景 C + §八-2：多图 log「P0 保持逐张独立卡」，组卡排 P1（并入 v1.0.1） | **多图组卡提到 P0**：一次投递 = 一张卡（N 图并列同卡） | 用户 2026-09-22 实测「我是一次性上传了三张照片，你分成了三个对话卡片」+ 拍板 A 方案 |
| §八-6：上传「保持逐张顺序上传（并发会破坏进度语义）」 | **改为一次投递一次请求**（多图一次发给后端，后端一次视觉调用） | 实测逐张串行：单张 10~17s × 3 张 → 客户端必然超时（REVIEW P1-多图2） |
| （原方案未涉及） | **新增投递级幂等 `Idempotency-Key`**：同键重发/重试返回首次结果，不重跑 AI、不重复落盘 | 实测超时重试造成同一张图两份（md5 相同）+ 记忆重复沉淀 |
| §五 P0 规则：纯图一律 log | 保持（本文档 P1「纯图看图兜底判定」仍未做，登记为后续） | — |
| §四场景 D P1：多图对话图区并排 | **已做**（随多图组卡一起） | 一卡多图口径的自然要求 |
