...
> **免费额度挂在「特定模型快照」上，通常与默认模型不通用（2026-09-13 实测核对）**：用户控制台显示 `fun-asr-flash-2026-06-15` 有 36,000 秒免费额度（到 2026-09-16 到期），但该模型官方说明是**短音频模型（可转写 5 分钟以内的音频）**，而本产品默认走 `paraformer-v2`（长音频异步转写）——**额度与默认模型不通用**，硬要吃掉它得把长音频切片转写再拼接（为几毛钱引入拼接/顺序/重试风险，不划算）。**结论：默认保持不变**；`adai.learn.asr.model` 可换模型，但换之前必须确认两件事：① 该模型支持长音频；② 与现有「录音文件异步转写」API 兼容，并同步改 `yuan-per-hour`。
