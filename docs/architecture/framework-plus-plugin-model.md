...
related:
  - ../rfc/20260816-framework-plus-plugin-model.md
...
| 交易三阶段模型 | **插件叠加的具体实例**（见 `../rfc/20260816-trading-agent-plugin-model.md`）|
...
|:-:|:-----|:-----|
| 1 | 本总纲（形态定义）| 定形态方向 |
| 2 | 交易 Agent 三阶段模型（`../rfc/20260816-trading-agent-plugin-model.md`）| 总纲在交易的实例化 |
| 3 | 差距与迁移（`../reference/framework-plugin-gap.md`）| 对账现状，列动刀点 |
