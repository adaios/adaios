...
| adai-admin 管理后台 | ⬜ | 规划中，范围待定（v0.2.0 末段）|
| 用户体系（多账号）| ⬜ | **v1.0.0**：简单登录（不做注册）+ 后台管理账号 |
...
- 用户体系可用：简单登录（不做注册）+ 后台管理账号（2026-08-02 确认纳入 v1.0.0）
