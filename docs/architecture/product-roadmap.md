...
| 用户体系（多账号）| ✅ 已实现 | 后端 ✅（账号存储 + #127 鉴权 + admin 管账号）+ 前端选号/切换链路 ✅（app 测试含选号/切换，2026-08-23 S-5 对拍修正原「顺延 v1.0.1」）；v1.0.0 单用户先行策略不变（`?userId=` 分流）|
...
| 候选 | 归属 | 前置依赖 |
|:-----|:-----|:---------|
| 多账号前端选号（MD12）| ✅ 已实现 | 后端账号体系 ✅ + #127 鉴权 ✅ + 前端选号/切换链路 ✅（2026-08-23 S-5 对拍修正，移出 v1.0.1 候选）|
| 图文一体数据层统一（RFC `20260815-media-event-unification`，approved）| v1.0.1 候选 | 层 1 展示层聚合 ✅（eb5e707）+ freeze §2.1 MINOR 变更 |
| 音频多模态记录 | 待定 | 录音/转写方案 |
...
- **用户体系**：后端账号体系 + admin 管理 ✅；前端选号/切换链路 ✅（2026-08-23 S-5 对拍修正原「顺延 v1.0.1」）；v1.0.0 单用户先行策略不变
