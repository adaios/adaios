...
id: mem_...
recordId: rec_...
cardId:                  # 关联对话卡 id（08-14：对话记忆记录卡片 id，删除时按 cardId 双匹配；非对话记忆可空）
...
### 2.24 节律 `rhythm/`（RFC 20260923 B 批，2026-09-23 新增）

**路径**：`data/{userId}/rhythm/YYYY/MM.md`（与 `todos/` 同级同构：`---` 围栏块 + 一行正文）。

**为什么**：周期习惯此前只能落进「记录 → 待办」唯一管道，被当成 OPEN 待办后**永远做不完**（习惯没有终点），
于是简报天天提醒（用户 2026-09-23：「阿呆 app 概览卡片，天天提醒我」「这是我的工作周期习惯，不是待办」）。
节律独立成条目后，周期由 RRULE 表示、只在命中日作为背景注入。
...
**兼容性**：`sourceRecordId` / `validUntil` 为**可选行**（缺行不炸）；未知 `status` 读作 **PAUSED**（保守：宁可少注入，不凭空多提醒）；
RRULE 脏数据按「不命中」处理（不炸简报）。**状态只有 active/paused/retired，没有 DONE**。
