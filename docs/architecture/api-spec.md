...
| 日期 | 版本 | 变更 |
|:----|:----|:------|
| 2026-09-23 | v3.90 | **分享追踪批（REVIEW P1-分享9；用户「我还没办法追踪呀……我看不到任何追踪信息，比如我分享了什么到阿呆，目前分别是什么情况了，是否完成了呢」）**——先查证：他 23:06:05 分享的公众号文章 **19 秒就跑完了**（23:06:06 抓取 2349 字 → 23:06:24 落卡《央行2025Q4货币政策报告要点》），而他 23:06:19 / 23:06:39 两次进学习页**什么都没看到**——缺的不是执行，是**过程可见性**。① **新端点 `GET /learn/digest/jobs`**：见下（落盘的账，滚动 50 条，**写失败 fail-open**）；② **Feed 注入 `type=digest` 条目**：「你给我的这篇，我正在读」→「读好了《…》」／「这篇我没读成」，主页就有回话；③ **App 学习页新增「整理进度」区**（进行中的全部 + 最近 3 条结局，`done` 点击直达卡片）；④ **六个节点各写一次账**（提交 / 抓到源 / 等确认 / 完成 / 失败 / 取消）——内存任务态会过期，账不会。**刻意不做**：不做排队（2026-09-23 用户拍板「暂时不用，单任务够」），第二条仍会被拒且如实告知。后端 **2188 → 2205**（+17：service 5 · 仓储 7 · Feed 3 · 端点 2）· app **412 → 419**（+7）· 端点 **165**（本批新增的 jobs 已由并发会话按 A1 代为登记标题，本行与上方的端点段补上正式说明） |
...
资金股份查询导入后返回券商口径账户：`{"assets":110504.88,"cash":292.88,"available":292.88,"withdrawable":292.88,"marketValue":110212.00,"pnl":15235.55,"todayPnl":0.0,"todayPnlSource":"broker","principal":150000,"snapshotDate":"2026-08-16","cashDate":"2026-08-16","cashNote":null}`。**字段语义（2026-08-16 修正）**：`pnl` = 持仓浮动盈亏（券商口径，非总盈亏）；**总盈亏 = `assets - principal`**（本金由用户提供，累计投入 15 万 → 当前总盈亏 -39,495.12）。顶层展示总资产/可用/可取/参考市值/当日盈亏/总盈亏/本金。数据依赖导入；收盘 15:05 自动更新行情相关字段（参考市值/当日盈亏/浮盈，P2-交易19 修订），现金/本金保持券商导入+转账推导。需 trading 插件（403）。
...
**`cashDate` / `cashNote`（v3.86，P2-交易69）**：现金这个数的**券商日期**与健康度提示。`cashDate` = 现金对应的券商快照日期（来自 `snapshot-anchor.json` 的 `cashImport`，无则 `""`）——**与 `snapshotDate` 不是一回事**：后者是收盘更新的日期（每个交易日都被刷新），而现金只在导入「资金股份查询」时才更新；两者混用会让人误以为手上的现金数是今天的（生产实据：09-11 导入真值 1,381.93 后，系统在两次导入之间漂到 24,101.01，而券商真值只有 414.86 → 总盈亏少报 2.37 万）。`cashNote` = 一句**可直接展示**的人话，`null` = 无需提示；三种情形按优先级：**负现金**（自证失败，生产 09-15 出现过 −6,093.97）> **无券商来源**（从没导过资金查询）> **过期**（距上次导入 > 7 天，阈值 `TradingAppService.CASH_STALE_DAYS`）。文案由后端给（单一真相源），双端只渲染。

**`principalNote`（v3.88，P2-交易66）**：本金置信度说明（`null` = 不提示）。`principal` 是**用户手填 + 已记录转账**推出来的，而 `transfers.json` 只有 2026-09 三笔、2025-04 建仓以来的出入金**零记录** → 「**总盈亏 = `assets` − `principal`**」不是账本能自证的数，而是「按用户报的本金算出来的」（生产真值 −43,819.14）。判据：`principal > 0` 且**转账记录最早一条晚于「今天 − 180 天」**（`TradingAppService.PRINCIPAL_TRAILING_DAYS`）→ 给一句人话并指向补记路径（资金页「转入 / 转出」）；未设本金（已有「设本金」引导）或转账覆盖够久 → `null`。
...
**body**（数组，可空）：
```json
[{"symbol":"600519","name":"贵州茅台","quantity":100,"avgCost":1400,"currentPrice":1478.5,
...
- **`currentPrice` 可选（2026-09-23 P2-交易65 修复）——券商「持仓股」导出的「现价」列**（生产实测 002428 现价 93.50 / 成本 41.58 / 300 股）。此前该列被**整列丢弃**、落库时拿 `avgCost` 顶替 → `positions.md` 的「现价」永远等于成本价；平时被运行时行情注入（`marketDataSource.quote()`）盖住，**行情源一挂就回退存储价 → 持仓页显示「0 盈亏 + 市值退回成本」的假象**（P1-交易62 的 K 线链路整段失效即同类风险；与 2026-08-16 修过的同型病）。语义：**仅在 > 0 时采用**；缺该字段/非正 → 后端**保留原有存储价**（绝不写回成本价，避免把真实行情抹成成本）；只有「新持仓且无任何现价来源」才回退 `avgCost`
...
### `GET /api/v1/learn/digest/jobs` — 整理任务清单（v3.90）

> **「我分享了什么、分别什么情况、成了没有」**（2026-09-23 分享追踪批，REVIEW P1-分享9）。
> 与 `/digest/status` 的分工：那个答**现在这一个**（内存执行态，结果 60 秒~30 分钟后清理），
> 这个答**落盘的账**——进行中 + 最近完成/失败/取消，**重启不丢、失败也留痕**。
> App 学习页的「整理进度」区与 Feed 里的 `type=digest` 条目读的是同一份。

**Query Parameters**

| 参数 | 类型 | 必填 | 说明 |
|:-----|:-----|:----:|:-----|
| `limit` | int | 否 | 最多返回几条（默认 20；≤0 用实现默认） |

**Response** `200`：

```json
{ "tasks": [
  { "id": "dtask_...", "url": "https://mp.weixin.qq.com/s/...", "sourceTitle": "央行报告",
    "platform": "mp.weixin.qq.com", "status": "done", "stage": null, "message": null,
    "type": "other", "title": "央行2025Q4货币政策报告要点", "topic": "中国货币政策",
    "submittedAt": "2026-09-23T23:06:05", "settledAt": "2026-09-23T23:06:24" } ] }
```

| 字段 | 说明 |
|:-----|:-----|
| `status` | `running`（正在抓/读/整理）/ `needs_confirmation`（等你拍板转写费）/ `done` / `failed` / `cancelled` |
| `stage` | 进行中阶段：`fetching` / `reading` / `transcribing` / `structuring` |
| `sourceTitle` · `platform` | 抓到的原文标题与来源（抓取成功前为 null）——**清单里要认得出「我发的是哪一篇」** |
| `title` · `type` · `topic` | `done` 时的结果卡片定位（前端据此点开全文） |
| `url` | 提交时的来源链接（仅粘贴素材 / 图片路径时为空串） |
| `submittedAt` · `settledAt` | 提交时刻 / 进入终态的时刻（ISO local date-time，秒精度） |

- 落盘 `data/{userId}/learn/_digest-tasks.json`，滚动保留最近 **50** 条（同 id 原地覆盖，状态推进不留重复行）
- **写失败 fail-open**（只记 WARN）：追踪账写不进去，绝不把已经成功的整理报成失败
- **受理失败不入账**：抢占任务位失败的那条根本没被受理，写进去就是假账
- `403`：learn 插件未启用
