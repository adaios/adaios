...
| 态 | 内容 | 状态 |
|:---|:-----|:-----|
| 语义近似 | 黄线 ≈ 长均线（默认 60 日可配）；白线 ≈ MA10；洗盘短线 ≈ 缩量长下影（`(high−low)/close` 大 + 下影 > 实体 + 缩量）；转型图（砖形图）≈ 趋势分段（N 日新高/新低翻转）| **第一版实现** |
...
| 15:10 买点扫描（`WatchlistBuyPointService`）| 扫描结果命中规则后，追加案例相似度 Top N（可选开关 `adai.trading.case.scan-match`，默认关——避免扫描变慢）；**范围=自选股+持仓** | P3 二期 |
| `GET /trading/buy-points` | 响应附 `caseMatches`（可选，向前兼容）| P3 二期 |
