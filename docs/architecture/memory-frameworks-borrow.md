...
## 十、2026-09-15 追加评估：三份记忆 MCP（DSH 官方文档推荐）

> 触发：用户读到 DSH 官方文档《连接第三方记忆 MCP 服务》，问「阿呆的记忆是否可以直接复用成熟 MCP 的能力；目前没依赖成熟工具，担心之后越来越多不好管理」。
> 结论：**维持 §九 C1 不采纳**——三份 MCP 与 basic-memory 同档、能力更弱；不引入，但把「何时才引入」落为三条量化触发线（§10.4）。

### 10.1 评估对象与文档自述的缺失

DSH 文档给出三份默认关闭的参考配置（均经 `@deepseek-ai/dsh-mcp-client` 桥接）：

| 方案 | 前置 | 检索能力 | 文档自述的缺失 |
|:-----|:-----|:---------|:--------------|
| Memorix | Node 22.18+ | 本地启发式（可选 provider） | 无 LLM/embedding 即可跑，即质量上限有限 |
| MCP Reference Memory | 无依赖 | **子串匹配，明确非语义** | 「不提供 embedding、自动摘要、冲突消解或遗忘策略」 |
| Engram | Go 1.25.10+ | 按 Git 项目隔离 | 定位为存储/项目选择，无记忆机制 |

**关键判据：三者缺失的恰是 AdaiOS 已实现的机制**（对比 §七 映射表）——遗忘与衰减、去重与 superseded 演变链、冲突消解、提炼摘要。引入不是升级而是降级，且新增一套与 **B2** 冲突的存储（`~/.memorix`、`~/.engram`、单 JSONL 文件，均不受 `data/` 隐私边界保护）。

### 10.2 与 §九 决策的关系（本次无变更）

- **C1（不引入框架本体）维持**：MCP 三件套属同一档，且比 Mem0/Letta/Zep 更轻、机制更弱。
- **B1（语义检索）不变**：仍为「先用现有全文搜索验证需求，再评估本地 embedding」；若引入，引入**索引库**（sqlite-vec / Lucene），不引入服务。
- **新增一条边界认知**：MCP 是「AI 客户端 ↔ 能力」协议，不是产品内部记忆层的形态——单机单用户进程模型、无鉴权/无用户隔离，多用户产品记忆不适用该协议。

### 10.3 现状规模基线（2026-09-15 实测，供增长判断）

| 项 | 数值 |
|:---|:-----|
| 核心代码 | 1207 行（`MemoryService` 853 + `Memory` 187 + `MemoryController` 108 + 小类） |
| 测试 | 607 行 / 33 个 `@Test` |
| 数据 | ~174 条（`memory/2026/08.md` 3124 行 172 条 + 07 月 2 条） |
| 消费方 | 61 处调用 / 22 个类 |
| 格式契约 | data-format-freeze.md §2.4 已冻结 |

> 判断：此规模属「健康的小模块」，未到需要外部底座的程度；「难管理」的真实风险来自数据格式脆性（裸 `---` 截断曾丢 17 条）、kind 失衡（decision 74%）、cardId 溯源缺失——均与「是否依赖成熟工具」无关。

### 10.4 引入触发线（三条，任一到齐再评估）

1. **语义检索**：标签+日期聚合确认不够用，且 `/api/v1/search` 全文验证过仍不够 → 上向量索引**库**（非 MCP / 非服务），真相源仍为 `data/`（守 B2）。
2. **规模**：单用户记忆达**万级**条目，或单月文件 > 5000 行 / > 300 条 → 先按月分片，再评估索引层。
3. **跨实体时序推理**：确需 graph 能力（Letta/Zep 强项）→ 作为**插件级能力**引入，不换底座（同 C3）。

### 10.5 本次未决（优先级高于任何工具选型）

`memory-fidelity.md` 的 **E-A（写侧保原话）仍为 draft**——8 月初无卡对话原话不可恢复，是已审计证实的真实失真源；kind 失衡（decision 74%）治理同源。此二项才是当下该拍的板。
