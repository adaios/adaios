...
AI 只做两件事：
1. 生成一句话摘要（≤20 字，第一人称"我"视角）
2. 打标签 + 判定 domain（life/trading/project，含关键词规则）

特点：temperature 低（0.3），输出 JSON，一条消息。
...
后端做三件事：
1. 让 AI 生成一段对话总结（≤40 字，第一人称，直接面向用户）
2. 保存为一条记录
