...
| 时间 | 推什么 |
|:--|:--|
| 09:15 | **早盘计划（含买点）**——持仓概览 + 自选买点**逐条四要素铁证**（你的历史 / 数字 / 规则原文 / 位置）|
| 12:00 | **午间知会**——只报事实与位置（是否到你设的止损位），**不催操作**；无异常不发 |
| 14:50 | **尾盘卖点**——只列触发卖出条件的持仓，四要素 + **账日期标注**；没触发不发 |
| 15:05 | 收盘账户自动更新（缺行情则整体跳过并推「未自动更新」） |
| ~~15:10~~ | ~~收盘买点扫描~~ **已并入 09:15 早盘**（2026-09-22 B1 起不再单独推） |
| 15:15 | 收盘交易日志确认（当日有候选才推） |
| 15:30（或同步完成时） | **收盘复盘**——**账同步完成后才出**（导入且 ≥15:00 即出）；没导则如实说「还没看到你的账」；正文 = 记账 / 账实 / 只记流水的 / 复盘 / 明天 |
